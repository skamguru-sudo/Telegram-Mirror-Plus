...
export default function JobCard({ initialJob }: JobCardProps) {
  ...
  const statusConfig = {
    ...
    running: { label: "RUNNING", variant: "running", icon: Activity, color: "text-primary", bg: "bg-primary" },
    completed: { label: "COMPLETED", variant: "success", icon: CheckCircle2, color: "text-success", bg: "bg-success" },
  ...
  return (
    <Card className={`overflow-hidden transition-colors ${isRunning ? "border-primary/50 shadow-[0_0_15px_rgba(30,111,217,0.1)]" : activeJob.status === "failed" ? "border-destructive/50 bg-destructive/5 shadow-[0_0_15px_rgba(255,0,0,0.1)]" : "border-border/50"}`}>
      <CardContent className="p-0">
        <div className="p-5 flex items-center justify-between gap-6">
          {/* Status & Identifiers */}
          <div className="flex items-center gap-4 flex-1">
            <div className={`flex items-center justify-center w-10 h-10 rounded-md ${config.bg}/10`}>
              <Icon className={`w-5 h-5 ${config.color} ${isRunning ? "animate-pulse" : ""}`} />
            </div>
          ...
          <div className="flex-1 flex flex-col justify-center">
            ...
            <Progress value={progress} className="h-1.5" indicatorColor={isRunning ? "bg-primary" : (activeJob.status === "completed" ? "bg-success" : activeJob.status === "failed" ? "bg-destructive" : "bg-muted-foreground")} />
          </div>
